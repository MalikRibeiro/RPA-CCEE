...
(() => {
  ...
  // ════════════════════════════════════════════════════════════════
  //  BLOCO 1 — PDF AUTO-DOWNLOAD (roda na aba de impressão / saw.dll)
  // ════════════════════════════════════════════════════════════════
  if (
    window.location.href.includes('saw.dll') &&
    (window.location.href.includes('downloadExportedFile') || window.location.href.includes('DashboardPrint'))
  ) {
    if (window.__cceeDownloader) return;
    window.__cceeDownloader = true;

    (async () => {
      // Aguarda o PDF carregar visualmente
      ...
      msg.style = "position:fixed;top:10px;left:10px;z-index:2147483647;background:#08080f;color:#00c896;padding:20px;border:2px solid #00c896;border-radius:10px;font-family:monospace;font-size:14px;box-shadow:0 0 20px rgba(0,200,150,0.5);";
      msg.innerHTML = `<b style="color:#fff">⚡ CCEE EXTRATOR:</b><br><br>Nome do arquivo: <span style="color:#fff">${nomeArquivo}</span><br><br><b>(Nome copiado para o Clipboard!)</b><br><br>1. Clique em 'Imprimir' ou 'Salvar'<br>2. Cole o nome (Ctrl+V)<br>3. Feche esta aba para continuar.`;
  ...
  // Clique aprimorado para evitar disparos múltiplos
  let lastClickTime = 0;
  function clickOracleBI(el) {
    if (!el) return false;
    const now = Date.now();
    if (now - lastClickTime < 2000) return false; // Debounce de 2 segundos
    lastClickTime = now;
  ...
  async function processarEmpresa(empresa) {
    ...
    // PASSO 6: Disparar PDF (Trava de disparo único)
    ...
    // PASSO 7: Aguardar aba de impressão abrir
    uiLog(`  [7/7] Aguardando aba de impressão...`, 'warn');
    ...
    window.__pdfLock = false; // Libera a trava após o processamento
  ...
  function buildPanelHTML(cfg) {
    ...
    <div id="ccee_panel">
      <div id="ccee_hdr">
        ...
        <span id="ccee_hdr_ver">v1.1.1</span>
      </div>
